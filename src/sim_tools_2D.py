...
def calc_energy_forces_mpi(pos, cell_dim, pos_indices, bond_indices, angle_indices, angle_bond_indices, vdw_indices, vdw_mat, virial_indicies, param):
	"""
	...
	for i in range(2):
		temp_xy[i][non_zero] -= nonbond_frc * (vdw_dist[i][non_zero] / vdw_r2[non_zero])
		for j in range(2):
			virial_tensor[i][j] += np.sum((temp_xy[i] * vdw_dist[i] * vdw_dist[j])[virial_indicies])
...
def calc_energy_forces(distances, r2, param, bond_matrix, vdw_matrix, verlet_list, bond_beads, dist_index, r_index):
	...
	for i in range(2):
		...
		for j in range(2):
			virial_tensor[i][j] += np.sum(np.triu(temp_xy[i] * distances[i] * distances[j]))
