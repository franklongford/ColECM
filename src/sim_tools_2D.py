...
def calc_energy_forces(dxdy, r2, bond_matrix, vdw_matrix, verlet_list, vdw_param, bond_param, angle_param, rc, bond_beads, dxdy_index, r_index):
	"""
	...
	if nbond > 0:
		...
		for i, sign in enumerate([1, -1]):
			f_beads_x[indices_half[i]] += sign * (bond_frc * dxdy[0][indices_half] / r_half)
			f_beads_y[indices_half[i]] += sign * (bond_frc * dxdy[1][indices_half] / r_half)

		"Bond Angles"

		try:
			"Make array of vectors rij, rjk for all connected bonds"
			vector = np.stack((dxdy[0][indices_dxy], dxdy[1][indices_dxy]), axis=1)
			n_vector = int(vector.shape[0])
	...
	temp_x[non_zero] += nonbond_frc * (dxdy[0][non_zero] / r2[non_zero])
	temp_y[non_zero] += nonbond_frc * (dxdy[1][non_zero] / r2[non_zero])
