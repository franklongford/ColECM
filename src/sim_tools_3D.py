...
def calc_energy_forces(distances, r2, param, bond_matrix, vdw_matrix, verlet_list, bond_beads, dist_index, r_index):
	...
	if nbond > 0:
		...
		r_full = np.sqrt(r2[indices_full])
		r_half = r_full[np.arange(0, nbond, 2)]
		...
		bond_frc = ut.force_harmonic(r_full, param['bond_r0'], param['bond_k0'])# * verlet_list_r0
		#bond_frc_1 = ut.force_harmonic(r_half, param['bond_r1'], param['bond_k1']) * verlet_list_r1

		f_beads_x[indices_full[0]] += bond_frc * distances[0][indices_full] / r_full
		f_beads_y[indices_full[0]] += bond_frc * distances[1][indices_full] / r_full
		f_beads_z[indices_full[0]] += bond_frc * distances[2][indices_full] / r_full
		...
		#f_beads_z[indices_half[i]] += sign * (bond_frc_1 * distances[2][indices_half] / r_half)
...
def calc_energy_forces_mpi(pos, cell_dim, pos_indices, bond_indices, frc_indices, angle_indices, angle_bond_indices, 
	...
	#frc_beads = np.zeros((n_dim, n_bead))
	f_beads_x = np.zeros((param['n_bead']))
	f_beads_y = np.zeros((param['n_bead']))
	f_beads_z = np.zeros((param['n_bead']))
	pot_energy = 0
	cut_frc = ut.force_vdw(param['rc']**2, param['vdw_sigma'], param['vdw_epsilon'])
	cut_pot = ut.pot_vdw(param['rc']**2, param['vdw_sigma'], param['vdw_epsilon'])
	virial_tensor = np.zeros((3, 3))
	nbond = bond_indices[0].shape[0]
	...
	if nbond > 0:
		...
		temp_frc = np.zeros((3,) + param['bond_matrix'].shape)
		...
		temp_frc[2][frc_indices] += bond_frc * pair_dist[2][bond_indices] / bond_r

		f_beads_x += np.sum(temp_frc[0], axis=1)
		f_beads_y += np.sum(temp_frc[1], axis=1)
		f_beads_z += np.sum(temp_frc[2], axis=1)
		...
		try:
			...
			f_beads_z[angle_indices.T[0]] += frc_angle_ij[ij_indices].T[2]
			f_beads_z[angle_indices.T[1]] += frc_angle_k.T[2]
			f_beads_z[angle_indices.T[2]] += frc_angle_ij[jk_indices].T[2]
	...
	for i in range(3):
		...
		for j in range(3):
			virial_tensor[i][j] += np.sum((temp_xyz[i] * pair_dist[i] * pair_dist[j])[virial_indicies])
...
def velocity_verlet_alg_mpi(pos, vel, frc, virial_tensor, param, pos_indices, bond_indices, frc_indices, angle_indices, angle_bond_indices, 
				vdw_coeff, virial_indicies, dt, sqrt_dt, cell_dim, comm, size, rank, NPT=False):
	"""
	...
	pot_energy, frc, virial_tensor = calc_energy_forces_mpi(pos, cell_dim, pos_indices, bond_indices, frc_indices, angle_indices, angle_bond_indices, 
								vdw_coeff, virial_indicies, param)
